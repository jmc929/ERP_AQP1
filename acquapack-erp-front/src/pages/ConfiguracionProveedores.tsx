...
const ConfiguracionProveedores = () => {
  ...
  const cargarDatos = async (tipo: TipoRegistro) => {
    try {
      setLoading(true);
      let endpoint = `http://localhost:4000/api/configuracion-proveedores/${tipo}`;
  ...
  const handleCrear = async () => {
    ...
    try {
      setLoading(true);
      const endpoint = `http://localhost:4000/api/configuracion-proveedores/${tabActivo}`;
  ...
  const handleGuardarEdicion = async () => {
    ...
    try {
      setLoading(true);
      const endpoint = `http://localhost:4000/api/configuracion-proveedores/${tabActivo}/${editandoId}`;
  ...
  const handleEliminar = async (id: number) => {
    ...
    try {
      setLoading(true);
      const endpoint = `http://localhost:4000/api/configuracion-proveedores/${tabActivo}/${id}`;
      const response = await fetch(endpoint, {
        method: "DELETE",
      });
  ...
  return (
    <PageContainer>
      <PageTitle title="Configuración de Proveedores" />
...
export default ConfiguracionProveedores;
